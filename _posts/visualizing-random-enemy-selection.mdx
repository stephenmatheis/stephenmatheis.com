...
I had a hard time visualizing how this would work. So I created this tool to experiment with different ways to select the next enemy.
...
I like both methods, but theirs is better in a couple of ways. It uses fewer tokens ([a cart can contain no more than 8192](https://www.lexaloffle.com/dl/docs/pico-8_manual.html#Specifications)). And the chance of selecting an ememy from from a back row means the player can't assume that the last enemy in the closest row will always be the next one selected.
